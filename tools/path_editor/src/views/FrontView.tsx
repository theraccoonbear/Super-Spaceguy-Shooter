...
// Shows exactly what the player sees: Z lateral horizontal, Y altitude vertical.
// Drag moves waypoints in Y and Z; X is inherited from the selected waypoint.
...
function w2s(wz: number, wy: number, w: number, h: number, cam: OrthoCamera) {
  return { sx: w / 2 + cam.panX + wz * cam.scale, sy: h / 2 + cam.panY - wy * cam.scale }
}
...
export function FrontView() {
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, cam: OrthoCamera) => {
    ...
    if (samples.length > 1) {
      ...
      if (path.standoff > 0.001) {
        ...
        samples.forEach(({ actual }, i) => {
          const { sx, sy } = w2s(actual.z, actual.y, w, h, cam)
          i === 0 ? ctx.moveTo(sx, sy) : ctx.lineTo(sx, sy)
        })
    ...
    if (path.orient === 'target') {
      const { sx: tx, sy: ty } = w2s(path.target.z, path.target.y, w, h, cam)
    ...
    // Waypoints — shown as dots (X depth not visible in this view)
    path.wps.forEach((wp, i) => {
      const { sx, sy } = w2s(wp.z, wp.y, w, h, cam)
    ...
    // Animated ship — shown as a cross (no meaningful 2D facing from this view without orientation)
  ...
  type DragState = { type: 'wp'; wpIdx: number; startSx: number; startSy: number; startWz: number; startWy: number }
                 | { type: 'pan'; startSx: number; startSy: number; startPanX: number; startPanY: number }
  const drag     = useRef<DragState | null>(null)
  const hasMoved = useRef(false)
  const drawRef  = useRef(redraw); drawRef.current = redraw
  ...
  function findNearWp(sx: number, sy: number, w: number, h: number): number {
    ...
    for (let i = 0; i < wps.length; i++) {
      const { sx: wx, sy: wy } = w2s(wps[i].z, wps[i].y, w, h, cam)
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (drag.current.type === 'wp') {
      ...
      const wps = useStore.getState().path.wps
      useStore.getState().setWp(wpIdx, { ...wps[wpIdx], z: startWz + dWz, y: startWy + dWy })
    }
  }, [])
  ...
  const onMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    if (!hasMoved.current && drag.current.type === 'pan') {
      const rect = getRect()
      const sx = e.clientX - rect.left, sy = e.clientY - rect.top
      const { wz, wy } = s2w(sx, sy, rect.width, rect.height, camRef.current)
  ...
  const onWheel = useCallback((e: React.WheelEvent) => {
    ...
    const sx = e.clientX - rect.left, sy = e.clientY - rect.top
    const cam = camRef.current
    const { wz, wy } = s2w(sx, sy, rect.width, rect.height, cam)
    const factor = e.deltaY > 0 ? 0.88 : 1.14
    cam.scale = Math.max(2, Math.min(80, cam.scale * factor))
