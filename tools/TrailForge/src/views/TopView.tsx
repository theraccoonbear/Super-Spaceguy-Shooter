...
export function TopView() {
  ...
  // Marquee selection rect (screen coords); drawn from inside draw().
  const marqueeRef = useRef<{ startSx: number; startSy: number; curSx: number; curSy: number } | null>(null)

  // Right-click context menu (wpIdx >= 0: on waypoint; wpIdx < 0: empty space with world coords)
  const [ctxMenu, setCtxMenu] = useState<{ x: number; y: number; wpIdx: number; wx?: number; wz?: number } | null>(null)
  const rightCtxRef  = useRef<{ idx: number; x: number; y: number; wx?: number; wz?: number } | null>(null)
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    const activeGhost = ghostRef.current ?? editGhost
    if (activeGhost !== null && ghostRef.current === null && editGhost !== null) {
  ...
  type DragState =
    ...
    | { type: 'mwp';       startSx: number; startSy: number; starts: Array<{ idx: number; wx: number; wz: number }> }
    | { type: 'marquee' }
  ...
  const onMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (e.button === 2) {
      ...
      if (nearIdx >= 0) {
        ...
        // Empty space: record world coords for "Add here" menu; also start pan so dragging works
        const { scale, worldPan: pan } = getCam(VIEW)
        const { wx, wz } = s2w(sx, sy, rect.width, rect.height, scale, pan)
        rightCtxRef.current = { idx: -1, x: e.clientX, y: e.clientY, wx, wz }
      }
      drag.current = { type: 'pan', startSx: sx, startSy: sy, startPan: { ...cam.worldPan }, startScale: cam.scale }
      return
    }
    ...
    if (idx >= 0) {
      ...
      if (ms.length > 0 && ms.includes(idx)) {
        // Node is part of multi-selection → drag all selected nodes together
        const wps = useStore.getState().path.wps
        pauseAfterCheckpoint()
        const starts = ms.map(i => ({ idx: i, wx: wps[i].x, wz: wps[i].z }))
        drag.current = { type: 'mwp', startSx: sx, startSy: sy, starts }
        const p = useStore.getState().path
        ghostRef.current = { path: { ...p, wps: [...p.wps] }, wpIdx: idx }
      } else {
        // Single node — clear multi-selection, select this node
    ...
    if (e.shiftKey) {
      ...
      // Plain drag on empty → clear multi-selection and pan
      useStore.getState().setMultiSel([])
      drag.current = { type: 'pan', startSx: sx, startSy: sy, startPan: { ...cam.worldPan }, startScale: cam.scale }
    }
  }, [behaviorsOpen])
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (drag.current.type === 'mwp') {
      ...
      for (const s of starts) {
        const wps = useStore.getState().path.wps
        useStore.getState().setWp(s.idx, { ...wps[s.idx], x: s.wx - dy / scale, z: s.wz + dx / scale })
      }
      return
    }
  ...
  const onMouseUp = useCallback((_e: React.MouseEvent) => {
    ...
    if (wasMarquee) {
      ...
      if (mr) {
        ...
        const found: number[] = []
        wps.forEach((wp, i) => {
          const { sx, sy } = w2s(wp.x, wp.z, rect.width, rect.height, scale, pan)
  ...
  const onDoubleClick = useCallback((e: React.MouseEvent) => {
    ...
    const rect = getRect()
    const sx = e.clientX - rect.left, sy = e.clientY - rect.top
    if (findNearWp(sx, sy, rect.width, rect.height) >= 0) return // on a node — ignore
    const { scale, worldPan: pan } = getCam(VIEW)
    const { wx, wz } = s2w(sx, sy, rect.width, rect.height, scale, pan)
      ...
      <canvas ref={cvRef} style={{ cursor: 'crosshair' }} tabIndex={0}
        ...
        onContextMenu={(e) => {
          ...
          if (rc !== null && !rightMovedRef.current) {
            setCtxMenu({ x: e.clientX, y: e.clientY, wpIdx: rc.idx, wx: rc.wx, wz: rc.wz })
          }
          rightCtxRef.current = null
        }} />
      {ctxMenu && (
        <CtxMenu x={ctxMenu.x} y={ctxMenu.y} wpIdx={ctxMenu.wpIdx}
          ...
          onAddHere={ctxMenu.wpIdx < 0 && ctxMenu.wx !== undefined ? () => {
            ...
            useStore.getState().addWp(
              { x: ctxMenu.wx!, y: sel >= 0 ? wps[sel].y : 0, z: ctxMenu.wz! },
              sel >= 0 ? sel : undefined,
            )
          } : undefined}
