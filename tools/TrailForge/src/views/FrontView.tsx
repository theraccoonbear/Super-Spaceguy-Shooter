...
import { drawBehaviorMarkers, drawRollIndicator } from './behaviorMarkers'
...
export function FrontView() {
  ...
  const [ctxMenu, setCtxMenu] = useState<{ x: number; y: number; wpIdx: number; wz?: number; wy?: number } | null>(null)
  const rightCtxRef  = useRef<{ idx: number; x: number; y: number; wz?: number; wy?: number } | null>(null)
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    if (path.transition && path.wps.length > 0) {
      ...
      const { sx: fx, sy: fy } = w2s(path.transition.from.z, path.transition.from.y, w, h, scale, pan)
    ...
    // Segment-track spans + craftRoll spans + trigger diamonds -- view-only
    // reference overlay; never hit-tested here (edited only on the behavior's
    // own timeline in BehaviorsPanel), so drawBehaviorMarkers' return value
    // (screen hit regions, meaningful only for interactive callers) is unused.
    if (behaviorsOpen) {
      drawBehaviorMarkers(ctx, samples, path, projectFor(w, h, scale, pan), hoveredBehavior, activeBehaviorTrack)
    }

    // ── Roll arc indicators at each waypoint ──────────────────────────────
    const crSegsAll = path.craftRollSegments ?? []
    if (crSegsAll.length > 0) {
      const nSegsWp = Math.max(segCount(path.wps, path.closed), 1)
  ...
  useEffect(() => {
    const fn = (e: Event) => {
      ...
      if (pane !== 'front' && pane !== 'all') return
      const canvas = cvRef.current
      if (!canvas) return
      const rect = canvas.getBoundingClientRect()
      framePoints('front', wps.length > 0 ? wps : useStore.getState().path.wps, rect.width, rect.height)
    }
  ...
  type DragState =
    ...
    | { type: 'mwp';       startSx: number; startSy: number; starts: Array<{ idx: number; wz: number; wy: number }> }
    | { type: 'marquee' }
  ...
  const onMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (e.button === 2) {
      ...
      if (nearIdx >= 0) {
        ...
        rightCtxRef.current = { idx: -1, x: e.clientX, y: e.clientY, wz, wy }
      }
      drag.current = { type: 'pan', startSx: sx, startSy: sy, startPan: { ...cam.worldPan }, startScale: cam.scale }
      return
    }

    // Behavior markers (craftRoll/segment-track spans, trigger diamonds) are
    // drawn in ortho views for reference ONLY -- never hit-tested or
    // draggable here. They're editable exclusively on the behavior's own
    // timeline in BehaviorsPanel; spatial waypoints are the only thing this
    // view's mouse handling ever picks or moves. See TrailForge's own
    // CLAUDE.md, "Behaviors Panel — Invariants" for the full contract.
    ...
    if (idx >= 0) {
      ...
      if (ms.length > 0 && ms.includes(idx)) {
        const wps = useStore.getState().path.wps
        pauseAfterCheckpoint()
        const starts = ms.map(i => ({ idx: i, wz: wps[i].z, wy: wps[i].y }))
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    // Behavior markers are view-only here (see onMouseDown) -- no hover
    // hit-test, no cursor change, no interaction with them in this view.
    ...
    if (drag.current.type === 'rotate') {
      ...
      const rotated = rotateAroundX(drag.current.snapshotWps, theta)
      useStore.getState().replaceWps(rotated)
      opHintRef.current = `↻ X  ${(theta * 180 / Math.PI).toFixed(1)}°`
      drawRef.current()
      return
    }
    ...
    if (drag.current.type === 'translate') {
      ...
      // FRONT: screen-X → world-Z, screen-Y → world-Y (inverted)
      const translated = translateWps(snapshotWps, 0, -dy / scale, dx / scale)
      useStore.getState().replaceWps(translated)
      opHintRef.current = `⇥ Y ${(-dy / scale).toFixed(1)}  Z ${(dx / scale).toFixed(1)}`
      drawRef.current()
      return
    }
    ...
    if (drag.current.type === 'mwp') {
      ...
      for (const s of starts) {
        const wps = useStore.getState().path.wps
        useStore.getState().setWp(s.idx, { ...wps[s.idx], z: s.wz + dx / scale, y: s.wy - dy / scale })
      }
      return
    }
  ...
  const onMouseUp = useCallback((_e: React.MouseEvent) => {
    ...
    if (wasMarquee) {
      ...
      if (mr) {
        ...
        const found: number[] = []
        wps.forEach((wp, i) => {
          const { sx, sy } = w2s(wp.z, wp.y, rect.width, rect.height, scale, pan)
  ...
  const onDoubleClick = useCallback((e: React.MouseEvent) => {
    ...
    const wps = useStore.getState().path.wps
    useStore.getState().addWp({ x: sel >= 0 ? wps[sel].x : 20, y: wy, z: wz }, sel >= 0 ? sel : undefined)
  }, [])
      ...
      <canvas ref={cvRef} style={{ cursor: 'crosshair' }} tabIndex={0}
        ...
        onContextMenu={(e) => {
          ...
          if (rc !== null && !rightMovedRef.current) {
            setCtxMenu({ x: e.clientX, y: e.clientY, wpIdx: rc.idx, wz: rc.wz, wy: rc.wy })
          }
          rightCtxRef.current = null
        }} />
      {ctxMenu && (
        <CtxMenu x={ctxMenu.x} y={ctxMenu.y} wpIdx={ctxMenu.wpIdx}
          ...
          onAddHere={ctxMenu.wpIdx < 0 && ctxMenu.wz !== undefined ? () => {
            const sel = useStore.getState().selected
            const wps = useStore.getState().path.wps
            useStore.getState().addWp(
              { x: sel >= 0 ? wps[sel].x : 20, y: ctxMenu.wy!, z: ctxMenu.wz! },
              sel >= 0 ? sel : undefined,
            )
          } : undefined}
