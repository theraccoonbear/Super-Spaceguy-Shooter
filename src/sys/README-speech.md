...
`speech_dict.txt` is embedded (`$EMBED:'assets/speech_dict.txt':'SPEECHDICT'`) and parsed by `SPK_LoadDict` into parallel arrays, sorted alphabetically by word (required — `SPK_DictFind` binary-searches them).
...
- Each of the 40 phonemes (39 ARPAbet + silence) has precomputed formant wavetables (`spkWaveLib`, built once by `SPK_BuildAllWaves` at `SPK_Init` — additive sine harmonics shaped by resonant peaks at each phoneme's F1/F2/F3 frequencies, three pitch variants for unstressed/primary/secondary stress). Building these at runtime per-phoneme would stall the real-time audio path; building them all upfront turns the hot path into a fast array copy.
- Vowels/sonorants read the wavetable directly; fricatives and stops add or substitute high-pass-filtered noise (cutoff varies by place of articulation, e.g. alveolar `T` is hissier than bilabial `P`); stops have a silent closure phase before a burst.
...
- `SPK_SyncToScroll(scrollPx, pxPerFrame)` adjusts `spkRateScale` so a queued utterance finishes roughly when the crawl text scrolls off-screen, clamped to 0.6×–1.2× to avoid audibly distorting phoneme quality.
